...
# ========== RATE LIMITING ==========
_api_call_times = []
_MAX_API_CALLS_PER_10_SECONDS = 80  # Conservative: 80% of 100 req/10s limit

async def check_api_rate_limit():
    """Ensure we don't exceed Polymarket API limits: 100 req/10s, 600 req/min."""
    global _api_call_times
    now = time.time()
    
    # Remove calls older than 10 seconds
    _api_call_times = [t for t in _api_call_times if now - t < 10]
    
    # If we're at the 10-second limit, wait
    if len(_api_call_times) >= _MAX_API_CALLS_PER_10_SECONDS:
        wait_time = 10 - (now - _api_call_times[0]) + 0.1
        logger.warning("Rate limit reached (%d/10s), waiting %.1f seconds", 
                      len(_api_call_times), wait_time)
        await asyncio.sleep(wait_time)
        # Clean up old entries after wait
        _api_call_times = [t for t in _api_call_times if now + wait_time - t < 10]
...
async def get_trade_info_from_api(tx_hash: str, maker_address: str, retry_count: int = 0) -> dict:
    """
    ...
    # Check if we recently failed to look up this tx (shorter cooldown with retries)
    if tx_hash in _failed_lookups:
        cooldown_time = 60 if retry_count == 0 else 300  # 1 min first fail, 5 min subsequent
        if time.time() - _failed_lookups[tx_hash] < cooldown_time:
            return None
    ...
    try:
        ...
        # Remove time filter entirely to avoid missing recent transactions
        params = {
            "limit": "500",  # Increased limit since no time filter
            "sortDirection": "DESC", 
            "user": maker_address,
            "type": "TRADE"
            # No startTs filter - get all trades for this maker
        }
        
        async with _http_semaphore:
            async with session.get(POLYMARKET_DATA_API_URL, params=params) as response:
                if response.status == 429:  # Rate limited
                    logger.warning("Rate limited on tx %s, maker %s - will retry", tx_hash[:8], maker_address[:8])
                    await asyncio.sleep(5)  # Brief pause before retry
                    return None  # Let caller retry with different maker
                
                if response.status != 200:
                    error_text = await response.text()
                    logger.warning("Data API returned %d for tx %s, maker %s: %s", 
                                  response.status, tx_hash[:8], maker_address[:8], error_text[:200])
                    if retry_count == 0:  # Don't mark as failed on first try
                        return None
                ...
                if not data:
                    logger.warning("API returned empty data for maker %s, tx %s", maker_address[:8], tx_hash[:8])
                
                # Look for matching transaction hash (handle truncated vs full hash formats)
                for trade in data:
                    api_tx_hash = trade.get("transactionHash", "")
                    # Match if either: exact match, API hash starts with subgraph hash, or subgraph hash starts with API hash
                    hash_match = (api_tx_hash == tx_hash or 
                                 api_tx_hash.startswith(tx_hash) or 
                                 tx_hash.startswith(api_tx_hash))
                    
                    
                    if hash_match and trade.get("type") == "TRADE":
                        trade_info = {
                ...
                # Since we already query without time filter, no fallback needed
                
                # Still no match found
                if data:
                    sample_txs = [t.get("transactionHash", "N/A")[:8] for t in data[:3]]
                    logger.warning("❌ No TRADE match for tx %s in %d results from maker %s. Sample TXs: %s", 
                                 tx_hash[:8], len(data), maker_address[:8], sample_txs)
                else:
                    logger.warning("❌ No trades found for maker %s", maker_address[:8])
                
                if retry_count > 0:  # Only mark as failed after retries
                    _failed_lookups[tx_hash] = time.time()
                return None
                
    except asyncio.TimeoutError:
        logger.warning("Timeout looking up tx %s with maker %s", tx_hash[:8], maker_address[:8])
        return None  # Let caller retry with different maker
    except Exception as e:
...
async def test_api_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Test Polymarket Data API health and connectivity."""
    chat_id = str(update.effective_chat.id)
    
    await context.bot.send_message(chat_id=chat_id, text="🔍 Testing Polymarket Data API...")
    
    try:
        # Test basic API connectivity with a known user
        session = await get_http_session()
        test_params = {
            "limit": "5",
            "sortDirection": "DESC",
        ...
        async with session.get(POLYMARKET_DATA_API_URL, params=test_params) as response:
            response_time = (time.time() - start_time) * 1000
            
            status_emoji = "✅" if response.status == 200 else "❌"
            status_text = f"{status_emoji} Status: {response.status}"
            
            if response.status == 200:
                data = await response.json()
                result_count = len(data) if isinstance(data, list) else 0
                status_text += f"\n📊 Returned {result_count} trades"
            ...
            rate_calls = len([t for t in _api_call_times if time.time() - t < 10])
            status_text += f"\n🚦 Rate limit: {rate_calls}/{_MAX_API_CALLS_PER_10_SECONDS} calls/10s"
        ...
        logger.error("API health test failed: %s", e)
    ...
    while not stop_event.is_set():
        try:
            ...
            for event in all_events:
                ...
                # FIXED: Use strict greater than to prevent boundary issues
            ...
            # Process individual qualifying transactions (REVERTED to original design)
            qualifying_txs_by_hash = {}  # Only transactions that meet threshold requirements
            
            # ORIGINAL LOGIC: Use subgraph data for threshold checks, API for enrichment
            for event in new_events:
                ...
                # Calculate subgraph notional value for threshold comparison
                subgraph_notional = maker_amt if maker_asset == "0" else taker_amt
                side = "BUY" if maker_asset == "0" else "SELL"
                
                # Check which chats need this alert based on SUBGRAPH data
                relevant_chats = []
                for chat_id in list(known_chats):
                    if chat_id not in chat_thresholds:
                        if chat_id not in missing_threshold_notified:
                            # Send notification asynchronously to avoid blocking
                            asyncio.create_task(send_telegram_message_to(chat_id, 
                                "👉 Please set your threshold with /setthreshold to receive alerts."))
                            missing_threshold_notified.add(chat_id)
                        continue

                    # RESTORED: Use subgraph data for threshold comparison
                    thresh = chat_thresholds[chat_id]
                    if subgraph_notional >= thresh:
                        relevant_chats.append(chat_id)
                
                # Only process transactions that meet at least one threshold
                if relevant_chats:
                    ...
                    qualifying_txs_by_hash[tx_hash]['events'].append({
                        'maker': maker,
                        'subgraph_notional': subgraph_notional,
                        'side': side
                    })
                    qualifying_txs_by_hash[tx_hash]['relevant_chats'].update(relevant_chats)
                    qualifying_txs_by_hash[tx_hash]['max_subgraph_notional'] = max(
                        qualifying_txs_by_hash[tx_hash]['max_subgraph_notional'], 
                        subgraph_notional
                    )
                    qualifying_txs_by_hash[tx_hash]['makers'].add(maker)

                # Mark as processed with timestamp
                processed_tx_hashes[tx_hash] = time.time()

            # Process each qualifying transaction (only those that met subgraph thresholds)
            if not qualifying_txs_by_hash:
                pass  # No qualifying transactions
            
            for tx_hash, tx_data in qualifying_txs_by_hash.items():
                # Skip if no chats actually qualify (shouldn't happen, but safety check)
                if not tx_data['relevant_chats']:
                    logger.warning("TX %s has no qualifying chats - skipping API call", tx_hash[:8])
                    continue
                
                api_calls_made += 1
                events_count = len(tx_data['events'])
                max_subgraph_notional = tx_data['max_subgraph_notional']
                makers_list = list(tx_data['makers'])
                
                logger.info("💰 Qualifying TX: %s $%.2f subgraph (%d events) - API call #%d", 
                           tx_hash[:8], max_subgraph_notional, events_count, api_calls_made)
                
                # Add 3-second delay for API indexing lag protection
                transaction_age = time.time() - tx_data['timestamp']
                if transaction_age < 6:
                    delay_needed = 6 - transaction_age
                    await asyncio.sleep(delay_needed)
                
                # Try API call for enrichment (market details, exact values)
                trade_info = None
                makers_tried = 0
                for maker in makers_list:
                    makers_tried += 1
                    trade_info = await get_trade_info_from_api(tx_hash, maker, makers_tried - 1)
                    if trade_info is not None:
                        break
                    
                    # Add small delay between maker attempts
                    if makers_tried < len(makers_list):
                        await asyncio.sleep(0.1)
                
                if trade_info is None:
                    # RETRY: Wait 5 seconds and try again for failed transactions
                    await asyncio.sleep(5)
                    
                    # Retry with all makers again
                    for maker in makers_list:
                        trade_info = await get_trade_info_from_api(tx_hash, maker, retry_count=1)
                        if trade_info is not None:
                            break
                        await asyncio.sleep(0.1)
                
                if trade_info is None:
                    # FALLBACK: Use subgraph data if API still fails after retry
                    logger.warning("⚠️ API failed after retry for TX %s - using subgraph data", tx_hash[:8])
                    trade_info = {
                        'slug': 'N/A',
                        'title': 'Unknown Market',
                        'outcome': 'UNKNOWN',
                        'side': tx_data['events'][0]['side'],
                        'price': 0,
                        'size': 0,
                        'usdcSize': max_subgraph_notional,  # Use subgraph as fallback
                        'type': 'TRADE',
                        'icon': '',
                        'name': 'Anonymous',
                        'proxyWallet': '',
                    ...
                    confirmed_trades += 1
                    api_trade_value = trade_info['usdcSize']
                    
                    logger.info("✅ TRADE Confirmed #%d: %s - %s %s | $%.2f API vs $%.2f subgraph", 
                               confirmed_trades, tx_hash[:8], trade_info.get('side'), 
                               trade_info.get('outcome'), api_trade_value, max_subgraph_notional)

                # Send alerts to all relevant chats (already qualified by subgraph thresholds)
                for chat_id in tx_data['relevant_chats']:
                    alerts_sent += 1
                    alert_value = trade_info['usdcSize']
                    is_fallback = trade_info.get('_fallback', False)
                    
                    logger.info("🚨 ALERT #%d → %s | %s %s | $%.2f %s", 
                               alerts_sent, chat_id, trade_info['side'], 
                               trade_info['outcome'], alert_value,
                               "(subgraph)" if is_fallback else "(API)")
                    
                    # Send alert with best available data
                    asyncio.create_task(_send_large_trade_alert(
                        chat_id, tx_hash, trade_info, alert_value
                    ))
...
def run_production_trade_monitor() -> None:
    ...
    application.add_handler(CommandHandler("apitest", test_api_health))
